...
async function main() {
  ...
  const logo1 = project.logoLine1 || "Batch";
  const logo2 = project.logoLine2 || "Scoring";
  const heroTitle = project.heroTitle || project.title;
  const footerTitle = project.footerProjectName || project.title || "Batch Scoring Pipeline";
  const footerSub =
    project.footerProjectSub ||
    "Validated CSV in · scored CSV out · CLI + tests";
  const buildId =
    args["build-id"] ||
    ...
    .replaceAll(
      ...
      escapeHtml(
        profile.footerTagline ||
          "Batch scoring, validation-first pipelines, and shippable Python services."
      )
    );
